...
- Um número pode ainda ser **formalmente reservado** por DEC vigente para um documento ainda não produzido, hipótese em que a lacuna na sequência é legítima e registrada. É o caso de **SPEC-003**, reservada por **DEC-003** ao eventual Contrato de Responsabilidade (só produzido se o projeto graduar do perfil solo para o de cinco peças); por isso as SPECs técnicas deste projeto começam em SPEC-004.
...
| **R-TAX-03** | Numeração sequencial dentro de cada tipo, sem reuso (§5). Lacunas só são legítimas se houver documento com status `superseded` ou `descartado` ocupando o número, **ou se o número estiver formalmente reservado por DEC vigente** (caso de SPEC-003, reservada ao Contrato de Responsabilidade por DEC-003). Número repetido, ou número faltante sem registro de descarte, supersedição ou reserva, sinaliza violação grave. | Cruzamento entre nome do arquivo, campo `documento:` do frontmatter e status, e leitura das DECs de reserva | Crítica |
...
| 2026-06-15 | v1 | Ajuste pré-aprovação (SPEC ainda em `proposto`, §7): a §5 e a R-TAX-03 passam a admitir **número formalmente reservado por DEC vigente** como lacuna legítima de numeração, ao lado de `superseded` e `descartado`. Acomoda a reserva do número SPEC-003 ao eventual Contrato de Responsabilidade decidida em **DEC-003** (perfil solo; SPECs técnicas a partir de SPEC-004). Sem mudança de versão. |
